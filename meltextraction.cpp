...
{ // First functions that is called
	...
	ierr = DMCreateGlobalVector(jr->surf->DA_SURF, &jr->surf->magmathick)       ;     CHKERRQ(ierr);
...
{// Last
	...
	ierr = VecDestroy(&MEPar->surf->lmagmathick)       			;     CHKERRQ(ierr);
	ierr = VecDestroy(&MEPar->surf->magmathick)       			;     CHKERRQ(ierr);
