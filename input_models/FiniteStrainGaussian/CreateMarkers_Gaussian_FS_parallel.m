...
dy  =   L/nump_y;
dz  =   H/nump_z;
x_left  =   -1; 	    % coord of the left margin
...
Phase(ind) = 0;
%Phase 1
z1 = 0.05*exp(-(((PARTICLES.x).^2)/0.1 + ((PARTICLES.y).^2)/0.1)) + 0.4;
z2 = 0.05*exp(-(((PARTICLES.x).^2)/0.1 + ((PARTICLES.y).^2)/0.1)) + 0.5;
ind = find(PARTICLES.z > z1 & PARTICLES.z <= z2);
Phase(ind) = 1;
