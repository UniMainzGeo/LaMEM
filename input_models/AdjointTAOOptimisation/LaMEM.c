...
int main(int argc, char **argv)
{
	...
	IOparam.mdI = 2;   // Number of indices
	...
	scal.utype         		= _NONE_;   // Or _SI_ or _GEO_
	scal.inp_mass 				= 1;
	scal.inp_time 				= 1;
	scal.inp_length 			= 1;
	scal.inp_temperature 		= 1;
	scal.inp_force 			= 1;
	...
	Par[1] = 0.0001;        // 5 (initial solution)
	...
	// UPPER BOUND
	VecGetArray(Ub,&Ubar);
	Ubar[0] = 10	/scal.density;
	Ubar[1] = 10	/scal.viscosity;
	VecRestoreArray(Ub,&Ubar);

	// LOWER BOUND
	VecGetArray(Lb,&Lbar);
	Lbar[0] = 0; 	// /scal.density;
	Lbar[1] = 0;	// /scal.viscosity;
	...
	// only compute the adjoint gradients or simply forward code
 	...
 	else if(IOparam.use == 3)
 	{
 		...
 		// 2. Set up Tao
 	 	...
 	 	ierr = TaoSetTolerances(tao,1e-15,1e-15,1e-15,1e-15,1e-15);	CHKERRQ(ierr);
