...
< FINITE DIFFERENCE STAGGERED GRID FORMULATION
  NOTE: to keep everything in the LaMEM framework, a number of 'tricks' are used here
  (material properties are e.g. computed with a FEM method)
 >
...
void ComputeVelocityLocal2Global_FDSTAG(
...
PetscErrorCode LaMEMVelPressureDACreate_FDSTAG( LaMEMVelPressureDA C )
{
	...
	/* set the element parameters - FDSTAG*/
	C->nnel        = 8;
	C->ngp_vel     = 1;		// with the FDSTAG method, we compute parameters @ the center; the real code will store 9 intp per element;
	...
	C->edof        = 7;		// 6 normal velocities & one P in the center
	C->edof_temp   = 8;
	...
	/* Set the operations */
	C->fp_ComputeElementIndex = &ComputeElementIndex_FDSTAG;

	C->fp_CreateStencilInGlobalStiffnessTemp = &CreateStencilInGlobalStiffnessTemp_FDSTAG;
	C->fp_GetTemperatureElement              = &GetTemperatureElement_FDSTAG;
	C->fp_SetValuesRHS_Temp                  = &SetValuesRHS_Temp_FDSTAG;
	C->fp_FindNearestNode                    = &FindNearestNode_FDSTAG;

	C->fp_ComputeVelocityLocal2Global = &ComputeVelocityLocal2Global_FDSTAG;
	C->fp_CreateStencilInGlobalRhs    = &CreateStencilInGlobalRhs_FDSTAG;
	C->fp_SetValuesRHS                = &SetValuesRHS_FDSTAG;

	C->fp_ComputeShapeFunctionVelocity = &ComputeShapeFunctionVelocity_FDSTAG;
	C->fp_ComputeShapeFunctionPressure = &ComputeShapeFunctionPressure_FDSTAG;


	C->fp_GetVelocityElement = &GetVelocityElement_FDSTAG;
	C->fp_GetElementCoords   = &GetElementCoords_FDSTAG;
