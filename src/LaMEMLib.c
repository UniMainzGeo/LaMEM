...
//	PetscLogDouble     cputime_start, cputime_start0, cputime_end, cputime_start_tstep, cputime_start_nonlinear;
	...
	if (user.ExplicitSolver == PETSC_TRUE) {
		...
		//user.stress_file = fopen(user.stress_file_name, "w" );
		//for(p=0; p<nproc; p++)
		//{
		//	asprintf(&fname, "strain_stress.%1.3lld",(LLD)p);
		//	user.stress_file[p] = fopen(fname, "w" );
		//	if(user.stress_file[p] == NULL) SETERRQ1(PETSC_COMM_SELF, 1,"cannot open file %s", fname);
		//}
		//asprintf(&fname, "strain_stress%1.3lld.%12.12e",jr.fs->dsz.rank,user.dt);
		//user.stress_file = fopen(fname, "w" );
		//if(user.stress_file == NULL) SETERRQ1(PETSC_COMM_SELF, 1,"cannot open file %s", user.stress_file_name);
