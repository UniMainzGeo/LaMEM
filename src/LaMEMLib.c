...
//	ierr = JacResReadCellPhases(&lm->jr, fb);  CHKERRQ(ierr);
	...
	// Gravity solver
	if ( lm->jr.ComputeGravity == 1 )
	{
		ierr = GRVSurveyCreate(&lm->fs, &lm->survey, fb); CHKERRQ(ierr);
	}
...
PetscErrorCode LaMEMLibSolve(LaMEMLib *lm, void *param)
{
	...
	while(!TSSolIsDone(&lm->ts))
	{
		...
		if(lm->jr.SkipStokesSolverTemperature == 1 && lm->jr.ts->istep <= lm->jr.SkipStokesSolverTemperatureTS-1)
		{
			PetscScalar       e2;

	    	ierr = JacResGetTempMat(&lm->jr);                       CHKERRQ(ierr);
	    	ierr = KSPSetOperators(lm->jr.tksp, lm->jr.Att, lm->jr.Att);    CHKERRQ(ierr);
	    	ierr = KSPSetUp(lm->jr.tksp);                           CHKERRQ(ierr);
	    	ierr = KSPSolve(lm->jr.tksp, lm->jr.ge, lm->jr.dT);             CHKERRQ(ierr);
	    	ierr = JacResUpdateTemp(&lm->jr);                       CHKERRQ(ierr);

	    	if(lm->jr.ctrl.actTemp == 1)
	    	{
	        	ierr = JacResGetTempRes(&lm->jr);         CHKERRQ(ierr);
	    		ierr = VecNorm(lm->jr.ge, NORM_2, &e2); CHKERRQ(ierr);
	    	}
		}
		else
		{
		...
		{	// Compute adjoint gradients
			ierr = AdjointObjectiveAndGradientFunction(&aop, &lm->jr, &nl, ((struct ModParam *)param), snes, &lm->surf); CHKERRQ(ierr);
		}
	...
	{	// Compute 'full' adjoint inversion
 			ierr = AdjointObjectiveAndGradientFunction(&aop, &lm->jr, &nl, IOparam, snes, &lm->surf); CHKERRQ(ierr);
	}
	...
	if(lm->jr.SaveGravity == 1)
	{
		// Save reference gravity field
		PetscInt         iter, i, j, k, nx, ny, nz, sx, sy, sz, rank;
		PetscScalar      rho;
		FILE            *fp;
		char            *fname;

		// get ranges of cells in n-dir
		GET_CELL_RANGE(nx, sx, lm->fs.dsx)
		GET_CELL_RANGE(ny, sy, lm->fs.dsy)
		GET_CELL_RANGE(nz, sz, lm->fs.dsz)
		iter = 0;
		...
		START_STD_LOOP
		{
			rho = lm->jr.svCell[iter++].svBulk.rho * lm->jr.scal->density;
			// write denstiy to file
			fprintf(fp, "%lf\n",rho);
		}
		END_STD_LOOP
		fclose(fp);
		PetscPrintf(PETSC_COMM_WORLD,"Saved reference density field ..\n\n");
	}
	else if	(lm->jr.ComputeGravity == 1 )	// Compute gravity
	{
		ierr = GRVCompute(&lm->fs, &lm->jr, &lm->survey); CHKERRQ(ierr);
	}
