...
PetscErrorCode Transition(Ph_trans_t *PhaseTrans, Marker *P, PetscInt PH1,PetscInt PH2, Controls ctrl,Scaling *scal, SolVarCell *svCell, PetscInt *ph, PetscScalar *T, PetscInt *InsideAbove, PetscScalar, JacRes *jr); //added JacRes for the dike air particle check
...
// edited the Check_Constant_Phase_Transition() for *ctrl instead of ctrl, same for Check_Clapeyron_Phase_Transition(), NEW for dike-phase box cekck for air particles 
//PetscErrorCode Check_AirPhaseRatio_Box_Transition(Material_t *mat, Controls ctrl, SolVarCell *svCell);
// NEW FOR DIKE, air check inside dike phase transition box, this would be an alternative way to checking the air particles as it is done right now, (ConstEqCtx *ctx not necessary?)
