...
PetscErrorCode PVOutCreateData(PVOut *pvout)
{
	...
	if(omask->MassEx)      OutVecCreate(&pvout->outvecs[iter++],    "MassEx",         scal->lbl_volume,           &PVOutWriteMassEx,     1);
	if(omask->Mass)         OutVecCreate(&pvout->outvecs[iter++],   "Mass",           scal->lbl_strain_rate,      &PVOutWriteMass,       1);
