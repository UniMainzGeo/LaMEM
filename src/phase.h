...
public:
	...
	PetscInt     Pd_rho_f;        // density from phase diagram?
	PetscInt     Pd_SV;           // Seismic velocitites from phase diagram?
	PetscInt     Pd_alpha;        // thermal expansivity from phase diagram?
	PetscInt     Pd_K;            // Compressibility from phase diagram?
	PetscScalar  Pd_Me;           // Melt from phase diagram?
	PetscScalar  Pd_mf;           // Constant melt fraction?
	PetscScalar  Me_Mu0;          // Viscosity of rock is 100 % molten
	PetscScalar  mf;              // CONSTANT melt fraction
};
...
struct PData
{
	...
	PetscScalar rho_f;
	// Seismic data
	PetscScalar rho_vs_v[max_num_ro][max_num_pd];
	PetscScalar Vs;
	PetscScalar rho_vp_v[max_num_ro][max_num_pd];
	PetscScalar Vp;
	// alpha
	PetscScalar rho_alpha_v[max_num_ro][max_num_pd];
	PetscScalar alpha;
	// K
	PetscScalar rho_K_v[max_num_ro][max_num_pd];
	PetscScalar K;
};
