...
PetscErrorCode PVOutWriteDevStress(OutVec* outvec)
{
	...
	#define GET_SXX { svCell = &jr->svCell  [iter++]; buff[k][j][i] = svCell->sxx; }
	#define GET_SYY { svCell = &jr->svCell  [iter++]; buff[k][j][i] = svCell->syy; }
	#define GET_SZZ { svCell = &jr->svCell  [iter++]; buff[k][j][i] = svCell->szz; }
	#define GET_SXY { svEdge = &jr->svXYEdge[iter++]; buff[k][j][i] = svEdge->s;   }
	#define GET_SYZ { svEdge = &jr->svYZEdge[iter++]; buff[k][j][i] = svEdge->s;   }
	#define GET_SXZ { svEdge = &jr->svXZEdge[iter++]; buff[k][j][i] = svEdge->s;   }
...
PetscErrorCode PVOutWriteJ2DevStress(OutVec* outvec)
{
	...
	#define GET_J2_STRESS_XY_EDGE { svEdge = &jr->svXYEdge[iter++]; s = svEdge->s; buff[k][j][i] = s*s;}
	#define GET_J2_STRESS_YZ_EDGE { svEdge = &jr->svYZEdge[iter++]; s = svEdge->s; buff[k][j][i] = s*s;}
	#define GET_J2_STRESS_XZ_EDGE { svEdge = &jr->svXZEdge[iter++]; s = svEdge->s; buff[k][j][i] = s*s;}
