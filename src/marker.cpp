...
{
	FDSTAG   *fs;
	PetscInt  nmarkx, nmarky, nmarkz, nummark, i;
	...
	for(i=0; i<actx->jr->dbm->numPhases; i++)
	{
...
PetscErrorCode ADVMarkSetTempFromFile(AdvCtx *actx, FB *fb)
{
	...
	for(imark = 0; imark < nummark; imark++)
	{
		...
		if(fb->PCT)
		{
			if(P->phase == fb->PCTphase1)
			{
				P->T = fb->PCTT1/chTemp;
			}
			else if(P->phase == fb->PCTphase2)
			{
				P->T = fb->PCTT2/chTemp;
			}
			else if(P->phase == fb->PCTphase3)
			{
				P->T = fb->PCTT3/chTemp;
			}
		}
...
PetscErrorCode LoadPhaseDiagram(AdvCtx *actx, Material_t  *phases, PetscInt i)
{
	...
	1 column = Vs [km/s]
	2 column = Vp [km/s]
	3 column = rho fluid [kg/m3]
	4 column = melt fraction []
	5 column = density [kg/m3]
	6 column = T [K]
	7 column = P [b]
	*/
	...
	else if(pd->rho_pdval[8][i_pd] == 7)   // density + mf + density_fluid + Vp + Vs
	{
		phases[i].Pd_Me    = 1;
		phases[i].Pd_rho_f = 1;
		phases[i].Pd_SV    = 1;
		fscanf(fp, "%lf %lf %lf %lf %lf %lf %lf,",&pd->rho_vs_v[0][i_pd],&pd->rho_vp_v[0][i_pd],&pd->rho_f_v[0][i_pd],&pd->Me_v[0][i_pd],&pd->rho_v[0][i_pd],&fl[0],&fl[1]);
		pd->rho_v[0][i_pd] /= scal->density;
		pd->rho_f_v[0][i_pd] /= scal->density;

		for (j=1; j<n; j++)
		{
			fscanf(fp, "%lf %lf %lf %lf %lf %lf %lf,",&pd->rho_vs_v[j][i_pd],&pd->rho_vp_v[j][i_pd],&pd->rho_f_v[j][i_pd],&pd->Me_v[j][i_pd],&pd->rho_v[j][i_pd],&fl[0],&fl[1]);
			pd->rho_v[j][i_pd] /= scal->density;
			pd->rho_f_v[j][i_pd] /= scal->density;
		}
	}
	else if(pd->rho_pdval[8][i_pd] == 9)   // density + mf + density_fluid + Vp + Vs + alpha + K
	{
		phases[i].Pd_Me    = 1;
		phases[i].Pd_rho_f = 1;
		phases[i].Pd_SV    = 1;
		phases[i].Pd_alpha = 1;
		phases[i].Pd_K     = 0;
		fscanf(fp, "%lf %lf %lf %lf %lf %lf %lf %lf %lf,",&pd->rho_K_v[0][i_pd],&pd->rho_alpha_v[0][i_pd],&pd->rho_vs_v[0][i_pd],&pd->rho_vp_v[0][i_pd],&pd->rho_f_v[0][i_pd],&pd->Me_v[0][i_pd],&pd->rho_v[0][i_pd],&fl[0],&fl[1]);
		pd->rho_v[0][i_pd]       /= scal->density;
		pd->rho_f_v[0][i_pd]     /= scal->density;
		pd->rho_alpha_v[0][i_pd] /= scal->expansivity;
		pd->rho_K_v[0][i_pd]      = (pd->rho_K_v[0][i_pd]*1e5) / scal->stress_si;

		for (j=1; j<n; j++)
		{
			fscanf(fp, "%lf %lf %lf %lf %lf %lf %lf %lf %lf,",&pd->rho_K_v[j][i_pd],&pd->rho_alpha_v[j][i_pd],&pd->rho_vs_v[j][i_pd],&pd->rho_vp_v[j][i_pd],&pd->rho_f_v[j][i_pd],&pd->Me_v[j][i_pd],&pd->rho_v[j][i_pd],&fl[0],&fl[1]);
			pd->rho_v[j][i_pd]       /= scal->density;
			pd->rho_f_v[j][i_pd]     /= scal->density;
			pd->rho_alpha_v[j][i_pd] /= scal->expansivity;
			pd->rho_K_v[j][i_pd]      = (pd->rho_K_v[j][i_pd]*1e5) / scal->stress_si;
		}
	}
	else
	{
