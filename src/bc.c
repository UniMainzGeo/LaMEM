...
PetscErrorCode BCReadFromOptions(BCCtx *bc)
{
	...
	if(bc->face)
	{
		...
		ierr = PetscOptionsHasName(NULL, NULL, "-bvel_influxmark", &bc->velmark.flg ); CHKERRQ(ierr);
		ierr = PetscOptionsHasName(NULL, NULL, "-bvel_alternate",  &bc->velmark.tflg); CHKERRQ(ierr);
