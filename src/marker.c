...
PetscErrorCode ADVMarkInit(AdvCtx *actx, UserCtx *user)
{
	...
	// Load phase diagrams for the phases where it is required
	for(i=0; i<actx->jr->numPhases; i++)
	{
		if(actx->jr->phases[i].Pd_rho == 1)
		{
			// To have the density already present in the first timestep call "ierr = ADVInterpMarkToCell(actx);" after loading the PD
			ierr = LoadPhaseDiagram(actx, i); CHKERRQ(ierr);
		}
	}
