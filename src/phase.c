...
PetscErrorCode DBMatReadPhase(DBMat *dbm, FB *fb)
{
	...
	m->Me_Mu0 = -2;
	m->mf     = -2;
	ierr = getScalarParam(fb, _OPTIONAL_, "Me_Mu0",      &m->Me_Mu0,   1, 1.0); CHKERRQ(ierr);
	if(m->Me_Mu0>-1){m->Pd_Me = 1;}else{m->Pd_Me = 0;}
	ierr = getScalarParam(fb, _OPTIONAL_, "mf",      &m->mf,   1, 1.0); CHKERRQ(ierr);
	if(m->mf>-1){m->Pd_mf = 0;}else{m->Pd_mf = 1;}
...
PetscErrorCode MatPropSetFromLibCall(JacRes *jr, ModParam *mod, FB *fb)
{
	...
	if(mod->use == 1 || mod->use == 3) {
		...
		for(im=0;im<mod->mdN;im++)
		{
			...
			// CONSTANT melt fraction
			else if(mod->typ[im] == _MFR_)
			{
				if(m->Pd_mf == 0)
				{
					m->mf = mod->val[im];
					PetscPrintf(PETSC_COMM_WORLD,"#    Mf[%lld] = %7.2f \n",(LLD)id,m->mf);
				}
				else
				{
					PetscPrintf(PETSC_COMM_WORLD,"Specify constant melt fraction to invert for it!\n");
				}
			}
			else
			{
