% --------------------------------------------------------%
%%%%% 2D shear localization model with random noise%%%%%
% --------------------------------------------------------%
...
Tage_OCP    =   200e6*SecYear; % Thermal age of overriding plate
...
% Continental crust
ind         =   find(Z>=z_cuc & Z<=z_air);
Phase(ind)  =   ContinentalUC;
% Continental crust
ind         =   find(Z>=z_clc & Z<z_cuc);
Phase(ind)  =   ContinentalLC;
% Subducting plate
ind         =   find(Z>=z_sp & Z<=z_oc & X>=x1+(Z-z1).*(x3-x1)/(z3-z1) & Temp<=1200);
Phase(ind)  =   SubductPlate;
% Overriding continental plate
ind         =   find(Z>=z_ocp & Z<=z_clc & X<=x2+(Z-z2)./tan(theta) & Temp<=1200);
