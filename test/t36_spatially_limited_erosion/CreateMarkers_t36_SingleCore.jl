# Marker setup for the t36_spatially_limited_erosion test.
#
# Builds a simple 2D (x-z) two-phase column: an air layer (phase 0) on top of a
# crust layer (phase 1), with the contact at z = surf_level = 10 km. The
# spatially-limited erosion model (erosion_model = 3 in the .dat) then erodes
# only the central window [er_x_min, er_x_max], carving a valley into the crust
# like a river incising its channel while the flanks are preserved.

# Load package that contains LaMEM I/O routines
using GeophysicalModelGenerator, SpecialFunctions

function CreateMarkers_t36_SingleCore(dir="./", ParamFile="spatially_limited_erosion.dat"; NumberCores=1, mpiexec="mpiexec", is64bit=false)
    ...
    # add an air phase (above the initial surface at z = 10 km)
    ...
    # add a crust phase (below the initial surface)
    add_box!(Phase, Temp, Grid,
                    xlim    = (-50.0, 50.0),
                    ylim    = (-1.0, 1.0),
                    zlim    = (-50.0, 10.0),
