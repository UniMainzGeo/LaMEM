...
my $found_TEST_TYPE;
    ...
    $diff_status = PerformComparison( "tolerance_diff_pl,1.0e-5",  "output/test-Diapir3D_Q1P0_TimeALE.0of1.output", "expected/test-Diapir3D_Q1P0_TimeALE.0of1.expected", "testing.diff"  );
}
...
##
sub get_mpiexec {
	my $file_name = $ENV{'PETSC_DIR'} . "/conf/petscvariables";	
	my $n = 0;
	my $FULLPATH_MPIEXEC = '';
	my $MPIEXEC = '';
	my $first = '';
